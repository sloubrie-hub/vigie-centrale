...
export default function Home() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setLiveItems(data.items || []); setSourceStates(data.sources || []); setLastRefresh(data.checkedAt || new Date().toISOString());
    } catch { setSourceStates([{ source: "Collecteur Vigie", status: "error", count: 0, detail: "La collecte n’a pas répondu" }]); }
    finally { setLoading(false); }
  };

  useEffect(() => {
    fetch("/api/veille", { cache: "no-store" })
      .then((response) => { if (!response.ok) throw new Error("Collecte indisponible"); return response.json(); })
      .then((data) => { setLiveItems(data.items || []); setSourceStates(data.sources || []); setLastRefresh(data.checkedAt || new Date().toISOString()); })
      .catch(() => setSourceStates([{ source: "Collecteur Vigie", status: "error", count: 0, detail: "La collecte n’a pas répondu" }]))
      .finally(() => setLoading(false));
  }, []);
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-status"><span className={loading ? "pulse loading" : "pulse live"} /><div><strong>{loading ? "Collecte en cours" : `${sourceStates.filter(s => s.status === "live").length} sources connectées`}</strong><small>{lastRefresh ? `Actualisé à ${new Date(lastRefresh).toLocaleTimeString("fr-FR", {hour:"2-digit",minute:"2-digit"})}` : "Initialisation"}</small></div></div>
      </aside>
      ...
      <section className="workspace">
        ...
        {view === "feed" ? <div className="content">
          <section className="hero">
            <div><p className="eyebrow">SAMEDI 18 JUILLET 2026</p><h1>Votre veille, sans bruit.</h1><p>Un seul espace pour repérer ce qui mérite vraiment votre attention.</p></div>
          ...
          <section className="connection-grid">{sourceStates.map(state => <article key={state.source} className={`connection ${state.status}`}><span>{state.status === "live" ? "LIVE" : state.status === "api" ? "API REQUISE" : "ERREUR"}</span><strong>{state.source}</strong><p>{state.detail}</p><b>{state.count} élément{state.count > 1 ? "s" : ""}</b></article>)}</section>
